...
//       else new AbstractTypeSymbol(this, pos, name)) setFlag newFlags
    ...
    def newStubSymbol(name: Name, missingMessage: String, pos: Position, isPackage: Boolean = false): Symbol = name match {
      case n: TypeName  => if (isPackage) new StubPackageClassSymbol(this, n, missingMessage, pos) else new StubClassSymbol(this, n, missingMessage, pos)
...
// ------ toString -------------------------------------------------------------------
  ...
  class StubPackageClassSymbol(owner0: Symbol, name0: TypeName, val missingMessage: String, override val pos: Position) extends PackageClassSymbol(owner0, owner0.pos, name0) with StubSymbol
